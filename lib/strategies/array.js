...
module.exports = function(schema, options) {
  ...
  var versionedSchema = new Schema({ refId: {type: refIdType, required: true}, created: Date, modified: Date, versions: [clonedSchema] });
  ...
  // Push updated version to versionModel -> version
  schema.post(['findOneAndUpdate', 'updateOne'], async function() {
    var self = this;
    ...
    var versionCopy = (await self.model.findOne(self.getQuery())).toObject()

    VersionedModel.findOne({ refId: versionCopy._id }, async function(err, versionedModel) {
      if (!versionedModel) {
        versionedModel = new VersionedModel({ refId: versionCopy._id, versions: [] });
      }
      ...
      versionCopy.refVersion = versionCopy.__v || 0;
      versionCopy._id = undefined;
      versionedModel.versions.push(versionCopy);
      if (versionedModel.versions.length > options.maxVersions) {
        versionedModel.versions.shift();
