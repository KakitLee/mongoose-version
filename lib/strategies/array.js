...
module.exports = function(schema, options) {
  ...
  schema.pre(['findOneAndUpdate', 'updateOne'], function(next) {
    ...
    VersionedModel.findOne({ refId: this._id }, async function(err, versionedModel) {
      if (!versionedModel) {
        versionedModel = new VersionedModel({ refId: self._id, versions: [] });
      }
      ...
      var versionCopy = await self.model.findOne(self.getQuery())

      versionCopy.refVersion = versionCopy.__v + 1 || 0;
      versionCopy._id = undefined;
